...
function astar.path ( start, goal, nodes, valid_node, dbgovrly, ls )
	...
	while #openset > 0 do
        if dbgovrly then -- ugly hack but whatever lol
            ls.begin()
            love.graphics.setColor(0, 0, 1, 1)
            for i, n in ipairs(openset) do
                love.graphics.rectangle('line', n.x*14-14, n.y*14, 14, 14)
            end
            love.graphics.setColor(1, 0, 0, 1)
            for i, n in ipairs(closedset) do
                love.graphics.rectangle('line', n.x*14-14, n.y*14, 14, 14)
            end
            love.graphics.setColor(0, 1, 0, 1)
            love.graphics.rectangle('line', goal.x*14-14, goal.y*14, 14, 14)
